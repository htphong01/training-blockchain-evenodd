...
describe("[Integration Test] Testing flow of the game", () => {
    const deployContractFixture = async () => {
        ...
        const cashContract = await Cash.deploy();

        const Ticket = await ethers.getContractFactory("Ticket");
        const ticketContract = await Ticket.deploy();

        const EvenOdd = await ethers.getContractFactory("EvenOdd");
        const evenOddContract = await EvenOdd.deploy(cashContract.address, ticketContract.address, {
    ...
    it("Play 3 games with 1 user -> User's ticket is expired -> Extends ticket -> Play 1 game with 1 user -> withdraw all token", async () => {
        const { owner, user1, cashContract, ticketContract, evenOddContract } = await loadFixture(
    ...
    it("Play 3 games with 3 user", async () => {
        const { owner, user1, user2, user3, cashContract, ticketContract, evenOddContract } = await loadFixture(
            deployContractFixture
        );
        await ticketContract.connect(user1).buy({
            value: ethers.utils.parseEther("0.1"),
